...
export class PaginationComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.dataService.sharedEvents.subscribe((res: any) => {
      ...
      this.maxResults = res?.res?.maxResults ? res.res.maxResults : this.maxResults;
      this.filter = res?.filter ? res.filter : this.filter;
      this.pages = this.generatePagesList(this.total);
    });
  }

  generatePagesList(total: number) {
    const pageList = [];
  ...
  onPageClick($event) {
    ...
    this.eventService.getEvents(this.startAt, this.maxResults, this.filter).subscribe(res => {
